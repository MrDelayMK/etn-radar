...
globalThis.fetch = async (url, opt) => {
  ...
  const u = new URL(url);
  if (u.searchParams.get("ids") === "electroneum") {
    return new Response(JSON.stringify([{ id: "electroneum", symbol: "etn", name: "Electroneum", market_cap_rank: 880, market_cap: 18e6 }]));
  }
...
pruef(r1.coins === 301 && abrufe.length === 3, "drei Abrufe: Top 300 und Electroneum");
pruef(abrufe[2].url.includes("ids=electroneum"), "der dritte Abruf fragt nur Electroneum");
...
pruef(plaetze.a === 1 && plaetze.b === 880 && plaetze.n === 301, "Plaetze 1 bis 300 plus Electroneum liegen in der Tabelle");

const r2 = await marktkapitalisierungen({}, db, { jetzt: jetzt + 3 * 3600000 });
pruef(r2.coins === 0 && abrufe.length === 3, "drei Stunden spaeter wird nicht erneut gefragt");
...
pruef(abrufe.length === 6, "am naechsten Tag wird wieder geholt");
...
pruef(/429/.test(fehler ?? "") && db.db.prepare("SELECT COUNT(*) n FROM coin_caps").get().n === 301, "bei 429 bleibt die alte Liste erhalten");
...
pruef(d.etn.rang === 880, "Rang von Electroneum wird mitgeliefert");
pruef(!d.coins.some((c) => c.i === "electroneum"), "Electroneum steht nicht in der Vergleichsliste");
...
pruef(leer.coins.length === 0 && leer.schnell.length === 0 && leer.stand === null && leer.etn.rang === null, "ohne Daten eine leere, aber gueltige Antwort");
